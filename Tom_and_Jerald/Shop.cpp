...
namespace {
	// -- Layout --
	const f32 k_title_y = 0.75f;   // Y position of the shop title
	const f32 k_credits_y = 0.60f;   // Y position of the credits/cheese display
	const f32 k_button_scale = 0.65f;   // Uniform scale applied to all upgrade buttons
	const f32 k_footer_y = -0.75f;  // Y position of the footer hints (ESC, click prompt)

	// -- Economy --
	const int k_upgrade_cost = 10;      // Cheese cost per upgrade purchase

	// -- Upgrade Button Y Positions (top to bottom) --
...
// Converts raw cursor pixel coords to normalized [-1, 1] screen space
...
// Returns true if the cursor (cx, cy) is within the bounding box of a text button
bool ShopState::isCursorOverText(const char* text, f32 center_y, f32 scale, f32 cursor_x, f32 cursor_y) const {
	...
	f32 padding = 0.08f; // must match the padding used in drawButtonBackground

	// Compute hit box edges
	f32 left = -(width + padding) * 0.5f;
	f32 right = (width + padding) * 0.5f;
	f32 bottom = center_y - padding * 0.5f;
	f32 top = center_y + height + padding * 0.5f;

	return cursor_x >= left && cursor_x <= right
		&& cursor_y >= bottom && cursor_y <= top;
}
...
// Draws a semi-transparent quad behind a button to give it a panel/card look
void ShopState::drawButtonBackground(const char* text, f32 center_y, f32 scale, f32 padding, f32 red, f32 green, f32 blue) const {
	...
	drawQuad(unit_square, center_x_world, center_y_world,
		button_width_world, button_height_world,
...
// Load  – one-time asset loading (font)
void ShopState::Load() {
	ASSETS::Init_Font();
	font_id = ASSETS::Font();
}

// Initialize  – per-session setup: mesh creation, file loads, camera reset
...
void ShopState::Update() {
	...
	// -- Purchase Handling (Left Click) --
	// For each button: check hover → check upgrade available → check funds → apply upgrade + save
	if (AEInputCheckTriggered(AEVK_LBUTTON)) {
		if (isCursorOverText(hp_text, k_health_button_y, k_button_scale, cx, cy)
			&& Upgrades_CanUpgradeHealth() && Credits_Spend(k_upgrade_cost)) {
			Upgrades_UpgradeHealth();
			Upgrades_WriteToFile("Assets/data/Upgrades.txt");
		}
		else if (isCursorOverText(size_text, k_size_button_y, k_button_scale, cx, cy)
			&& Upgrades_CanUpgradeSize() && Credits_Spend(k_upgrade_cost)) {
			Upgrades_UpgradeSize();
			Upgrades_WriteToFile("Assets/data/Upgrades.txt");
		}
		else if (isCursorOverText(cap_text, k_fuel_cap_button_y, k_button_scale, cx, cy)
			&& Upgrades_CanUpgradeFuelCap() && Credits_Spend(k_upgrade_cost)) {
			Upgrades_UpgradeFuelCap();
			Upgrades_WriteToFile("Assets/data/Upgrades.txt");
		}
		else if (isCursorOverText(spawn_text, k_fuel_spawn_button_y, k_button_scale, cx, cy)
			&& Upgrades_CanUpgradeFuelSpawn() && Credits_Spend(k_upgrade_cost)) {
			Upgrades_UpgradeFuelSpawn();
			Upgrades_WriteToFile("Assets/data/Upgrades.txt");
		}
		else if (isCursorOverText(restore_text, k_fuel_restore_button_y, k_button_scale, cx, cy)
			&& Upgrades_CanUpgradeFuelRestore() && Credits_Spend(k_upgrade_cost)) {
...
void ShopState::Draw() {
	...
	// -- Screen Clear & Render State Reset --
	...
	// -- Build Button Label Strings (mirrors Update for accurate hit-testing) --
	char hp_text[128], size_text[128], cap_text[128], spawn_text[128], restore_text[128];
	sprintf_s(hp_text, "UPGRADE HEALTH (+5%%) [LEVEL %d/%d]", Upgrades_GetHealthLevel(), k_health_upgrade_max_level);
	sprintf_s(size_text, "UPGRADE SIZE (-0.1) [LEVEL %d/%d]", Upgrades_GetSizeLevel(), k_size_upgrade_max_level);
	sprintf_s(cap_text, "UPGRADE FUEL CAP (+10%%) [LEVEL %d/%d]", Upgrades_GetFuelCapLevel(), k_fuel_upgrade_max_level);
	...
	// -- Upgrade Buttons: background panel + label --
...
// Free  – per-session teardown: release mesh, hide cursor
void ShopState::Free() {
